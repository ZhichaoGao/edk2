...
#define DUMP_HEX(ErrorLevel,                                                      \
  ...
  do {                                                                            \
    if (DebugPrintEnabled () && DebugPrintLevelEnabled (ErrorLevel))  {           \
      ...
      VA_START(ListMarker, LinePrefixFormat);                                     \
                                                                                  \
      ASSERT (LocalOffset < LocalDataSize);                                       \
                                                                                  \
      LocalDataSize -= LocalOffset;                                               \
      DebugPrint (ErrorLevel, "Memory Address: %16.16X\r\n", DataToDump);         \
                                                                                  \
      DataToDump += LocalOffset;                                                  \
      while (LocalDataSize != 0) {                                                \
        ...
        Str[DumpHexIndex]      = 0;                                               \
                                                                                  \
        DebugVPrint (ErrorLevel, LinePrefixFormat, ListMarker);                   \
